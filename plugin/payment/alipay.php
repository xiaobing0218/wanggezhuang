...
class alipay extends abstract_payment
{
    public function create_pay_url($args)
    {
         ...
         * 解析数组
         */
        $order_num=explode(',',$args);
        $order=array();
        $sum=0;
        foreach ($order_num as $k=>$v){
            $order[$k]=(new order_model())->find(array('order_id'=>$v));
            $order_id=$v;
        }
        ...
        (
            ...
            'subject'        => "{$GLOBALS['cfg']['site_name']}订单-{$order_num[0]}",
            'total_fee'      => $sum,
        ...
        if($this->device == 'mobile')
        {
            ...
            $params['show_url'] = url('mobile/order', 'view', array('id' => $args));
        }
        else
        {
            ...
            $params['show_url'] = url('order', 'view', array('id' => $args));
        }
