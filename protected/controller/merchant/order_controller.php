    ...
    public function action_view()
    {
        ...
        if ($order = $order_model->find($condition)) {
            ...
            if ($this->log_list = $log_model->find_all($condition, 'dateline DESC')) {
                $this->admin_list = $vcache->merchant_model('indexed_list');
            ...
            switch (request('step')) {
                case 'consignee':
                    ...
                    if ($order['order_status'] == 1 || $order['order_status'] == 2) {
                        ...
                        if (TRUE === $verifier) {
                            var_dump($consignee_model->update(array('order_id' => $order_id), $data));

                            if ($consignee_model->update(array('order_id' => $order_id), $data) > 0) {
                                $cause = trim(request('cause', ''));
                                $log_model = new order_log_model();
                                $log_model->record($order_id, 'consignee', $cause, $merchant_id);
                            } else {
                                $errno = 1;
                            }
                        } else {
                ...
                case 'amount': //更改金额
                    ...
                    if ($order['order_status'] == 1) {
                        ...
                        if ($order_model->update($condition, array('order_amount' => $order_amount)) > 0) {
                            ...
                            $log_model = new order_log_model();
                            $log_model->record($order_id, 'amount', $cause, $merchant_id);
                        } else {
                            $errno = 1;
                        }
                    } else {
                ...
                case 'cancel': //取消交易
                    ...
                    if ($order['order_status'] == 1) {
                        if ($order_model->update($condition, array('order_status' => 0)) > 0) {
                            ...
                            $cause = trim(request('cause', ''));
                            $log_model = new order_log_model();
                            $log_model->record($order_id, 'cancel', $cause, $merchant_id);
                        } else {
                            $errno = 1;
                        }
                    } else {
                ...
                case 'resume': //恢复被取消交易
                    ...
                    if ($order['order_status'] == 0) {
                        if ($order_model->update($condition, array('order_status' => 1)) > 0) {
                            ...
                            $cause = trim(request('cause', ''));
                            $log_model = new order_log_model();
                            $log_model->record($order_id, 'resume', $cause, $merchant_id);
                        } else {
                            $errno = 1;
                        }
                    } else {
