...
class favorite_controller extends general_controller
{
    public function action_list()
    {
        ...
        if ($list){
            foreach ($list as $k=>$v){
                $merchant_id=(new merchant_model())->find(array('user_id'=>$v['merchant_id']),'','user_id,company_name,logo');
                $list[$k]['company_name']=$merchant_id['company_name'];
    ...
    public function action_addstore()
    {
        $user_id = $this->is_logined();
        $user_store_model = new user_store_model();
        if($user_store_model->add($user_id, (int)request('user_id', 0)))
        {
            echo json_encode(array('status' => 'success'));
        }
        else
    ...
    public function action_deletestore()
    {
        ...
        if($id)
        {
            $fav_model = new user_store_model();
            if(is_array($id))
            {
                foreach($id as $v) $fav_model->delete(array('merchant_id' => (int)$v, 'user_id' => $user_id));
            }
            else
            {
                $fav_model->delete(array('merchant_id' => (int)$id, 'user_id' => $user_id));
            }
            echo json_encode(array('status' => 'success'));
        }
        else
