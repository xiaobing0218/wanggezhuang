...
class goods_review_controller extends general_controller
{
    public function action_index()
    {
        ...
        if(request('step') == 'search')
        {
            ...
            $binds = array();
                 ...
                 if($shop_id != '')
                 {
                     $where_2 .= ' AND a.merchant_id = :goods_id';
                     $binds[':goods_id'] = (int)$shop_id;
                 }
            if($goods_id != '')
            {
            ...
            if($total[0]['count'] > 0)
            {
                ...
                $sql = "SELECT a.review_id, a.order_id, a.rating, a.content, a.created_date, a.status, a.replied,a.merchant_id,
                        ...
                        LEFT JOIN {$GLOBALS['mysql']['MYSQL_DB_TABLE_PRE']}merchant AS d
                        ON a.merchant_id = d.user_id
                        {$where_2}
                        ORDER BY {$sort} {$limit}
                       ";
        ...
        else
        {
            if(!empty($goods_id))
            {
                ...
                $sql = "SELECT a.goods_id,a.merchant_id, a.goods_name, a.goods_image,
            ...
            $this->shop=(new merchant_model())->find_all();
    ...
    public function action_view()
    {
        $id = (int)request('id', 0);
        $review_model = new goods_review_model();
        $sql = "SELECT a.review_id, a.order_id,a.merchant_id, a.rating, a.content, a.created_date, a.status, a.replied,
