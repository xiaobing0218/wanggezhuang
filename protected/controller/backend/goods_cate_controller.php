    ...
    public function action_edit()
    {
        if(request('step') == 'submit')
        {
            ...
            if(TRUE === $verifier)
            {
                $id = (int)request('id', 0);
                if ($data['state']!=1){
                    $good=(new goods_model())->find_all(array('cate_id'=>$id));
                    if ($good){
                        $goods=(new goods_model())->update(array('cate_id'=>$id),array('distinguish'=>0));
//                        if (!$goods){
//                            $this->prompt('error', '更新商品分类失败', url($this->MOD.'/goods_cate', 'index'));
//                        }
                    }
                }else{
                    $good=(new goods_model())->find_all(array('cate_id'=>$id));
                    if ($good){
                        $goods=(new goods_model())->update(array('cate_id'=>$id),array('distinguish'=>1));
//                        if (!$goods){
//                            $this->prompt('error', '更新商品分类失败', url($this->MOD.'/goods_cate', 'index'));
//                        }
                    }
