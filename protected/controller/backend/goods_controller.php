...
//                $binds[':kw'] = '%'.$kw.'%';
            ...
            if($admin_info['brand_id']){
                $where .= ' AND brand_id = :brand_id';
                $binds[':brand_id'] = $admin_info['brand_id'];
            }
            ...
            if($total[0]['count'] > 0)
            {
                $fields = 'merchant_id,goods_id, goods_name, goods_sn, now_price, stock_qty, created_date, newarrival, recommend, bargain, status, rowNo';
        ...
        else
        {
            ...
            if($admin_info['brand_id']){
                $brand_list = $brand_model->indexed_list_where($admin_info['brand_id']);
            }
        ...
        switch(request('step'))
        {
            ...
            default:
                ...
                if($this->rs = $goods_model->find($condition))
                {
                    ...
                    //获取购买选项
                    $optional_type_model = new goods_optional_type_model();
                    $this->opt_type_list = $optional_type_model->indexed_list($this->rs['merchant_id']);
                    $opt_model = new goods_optional_model();
                    $this->opt_list = $opt_model->get_goods_optional($goods_id,$this->rs['merchant_id']);
    ...
    public function action_image()
    {
        switch(request('step'))
        {
            case 'list':
                ...
                $path = "upload/goods/{$dir}/{$merchant_id}/";
