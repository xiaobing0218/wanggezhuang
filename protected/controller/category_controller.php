...
//            $this->bargain = vcache::instance()->goods_model('find_goods', array(array('cate' => $id, 'bargain' => 1), 5), $GLOBALS['cfg']['data_cache_lifetime']);
            ...
            $cate=(new goods_cate_model())->find_all(array('parent_id'=>$id,'state'=>1));
            $this->cate=$cate;
            if($cate){
                $this->count=count($cate);
            }else{
                $this->count=0;
            }
            ...
            $goods_list = $goods_model->find_goods($conditions, array(request('page', 1), $GLOBALS['cfg']['cate_goods_per_num']));
            if ($goods_list){
                foreach ($goods_list as $k => $v) {
                  $goods_list[$k]['merchant_info']=(new merchant_model())->find(array('user_id'=>$v['merchant_id']));
                }
            }
