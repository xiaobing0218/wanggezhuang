...
class consignee_controller extends general_controller
{
    ...
    {
        $user_id = $this->is_logined();
        $store=new user_store_model();
        $merchant=$store->find_all(array('user_id'=>$user_id));
        if ($merchant){
            foreach ($merchant as $k=>$v){
                $info=(new merchant_model())->find(array('user_id'=>$v['merchant_id']));
                $merchant[$k]['abbreviation']=$info['abbreviation'];
                $merchant[$k]['logo']=$info['logo'];
            }
        }
        $this->merchant=$merchant;
        $this->compiler('user_favorite_shop.html');
    }
