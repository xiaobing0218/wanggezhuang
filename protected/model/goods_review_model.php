...
class goods_review_model extends Model
{
    ...
    (
        ...
        (
            ...
            'min_length' => array(5, '评价内容不能少于5个字符'),
    ...
    public function get_goods_reviews($goods_id, $rating_type = 0, $limit = null)
    {
        ...
        if($total[0]['count'] > 0)
        {
            ...
            $sql = "SELECT a.review_id,a.status, a.rating, a.content, a.created_date, a.replied, b.user_id, b.username, b.avatar
     ...
     * 获取商品评分统计
     */
    public function get_rating_stats($goods_id,$status='')
    {
            if ($status){
        ...
        unset($good);
        if ($status){
            $arr = $this->query($sql, array(':goods_id' => $goods_id,':status'=>$status));
        }else{
            $arr = $this->query($sql, array(':goods_id' => $goods_id));
        }
        if($arr)
        {
            $numerator = 0;
