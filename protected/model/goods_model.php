...
class goods_model extends Model
{
    ...
    {
        $where = 'WHERE status = 1 AND distinguish = 1';
        $binds = array();
        if (!empty($conditions['cate'])) {
            $cate_id = $conditions['cate'];
            $cate_info = (new goods_cate_model())->find(['cate_id' => $cate_id]);
            $c_list = (new goods_cate_model())->find_all(['parent_id' => $cate_info['cate_id']]);

            if ($c_list) {
                foreach ($c_list as $item) {
                    $cate_id .= ',' . $item['cate_id'];
                    $f_c_list = (new goods_cate_model())->find_all(['parent_id' => $item['cate_id']]);
                    if ($f_c_list) {
                        foreach ($f_c_list as $item_f) {
                            $cate_id .= ',' . $item_f['cate_id'];
                        }
                    }
                }
            }
        ...
        if (!empty($conditions['merchant_id'])){
            $where .= ' AND merchant_id = '.$conditions['merchant_id'];
        }

        if (!empty($conditions['bargain'])) {
            $where .= ' AND bargain = 1';
        }
        if (!empty($conditions['g_p'])) {
            $where .= ' AND g_p = 1';
        }
        ...
        if (!empty($conditions['merchant_id'])) {
            ...
            $binds[':merchant_id'] = $conditions['merchant_id'];

        }
        if (isset($conditions['kw']) && $conditions['kw'] != '') {
            $conditions['kw'] = sql_escape($conditions['kw']);
            if ($GLOBALS['cfg']['goods_fulltext_query'] == 1) {
//                $where .= ' AND MATCH (goods_name,meta_keywords,goods_sn) AGAINST (:kw IN BOOLEAN MODE)';
//                $binds[':kw'] = $conditions['kw'];
        ...
        if ($total[0]['count'] > 0) {
            ...
            $fields = 'distinguish,merchant_id,goods_id, cate_id, brand_id, goods_name, original_price, goods_brief, now_price, goods_image';
    ...
    public function set_search_filters($conditions)
    {
        ...
        $sql = "SELECT cate_id, cate_name FROM {$GLOBALS['mysql']['MYSQL_DB_TABLE_PRE']}goods_cate
                WHERE parent_id ={$conditions['cate']}
                ORDER BY seq ASC
               ";
        ...
        if (!$filters['cate']){
            $one=(new goods_cate_model())->find(array('cate_id'=>$conditions['cate']));
            $sql = "SELECT cate_id, cate_name FROM {$GLOBALS['mysql']['MYSQL_DB_TABLE_PRE']}goods_cate
                WHERE parent_id ={$one['parent_id']}
                ORDER BY seq ASC
               ";
            $filters['cate'] = $this->query($sql, $binds);
        }
    ...
    public function get_history($limit = 10)
    {
        if ($cookie = request('FOOTPRINT', null, 'cookie')) {
            ...
            $sql = "SELECT merchant_id,goods_id, goods_name, original_price, now_price, goods_image
                    FROM {$this->table_name}
                    WHERE goods_id in ({$marks})
                   ";
            if (!empty($limit)) $sql .= " LIMIT {$limit}";

               $goods= $this->query($sql, $ids);
                if ($goods){
                    foreach ($goods as $k=>$v){
                        $merchant_id=(new merchant_model())->find(array('user_id'=>$v['merchant_id']),'','user_id,abbreviation,logo');
                        $goods[$k]['abbreviation']=$merchant_id['abbreviation'];
                        $goods[$k]['logo']=$merchant_id['logo'];
                    }
                }
    ...
    public function get_cart_items(array $items)
    {
        ...
        $marks = str_repeat('?,', count($ids) - 1) . '?';
        $sql = "SELECT merchant_id,goods_id, goods_name, now_price, goods_image, goods_weight, stock_qty
                FROM {$this->table_name} WHERE status = 1 AND goods_id in ({$marks}) GROUP BY merchant_id";
        $goods = $this->query($sql, $ids);


        if ($goods) {
            foreach ($goods as $k => $v) {
                $merchant[$k] = (new merchant_model())->find(array('user_id' => $v['merchant_id']), '', 'abbreviation,user_id,logo');
            }
            foreach ($merchant as $k => $v) {
                $sql = "SELECT merchant_id,goods_id, goods_name, now_price, goods_image, goods_weight, stock_qty
                FROM {$this->table_name} WHERE status = 1 AND merchant_id={$v['user_id']} AND goods_id in ({$marks}) ";
                $merchant[$k]['commodity'] = $this->query($sql, $ids);
            }
        }
        if ($merchant) {
            foreach ($merchant as $k1 => $v1) {

                $goods_map = $v1['commodity'];
                if ($goods_map) {
                    unset($ids, $marks);
                    ...
                    $merchant[$k1]['commodity'] = $res['items'];
                    $merchant[$k1]['amount']=$res['amount'];
                    $merchant[$k1]['qty']=$res['qty'];
                    $merchant[$k1]['weight']=$res['weight'];
                }
            }
            $amount = 0;
            $qty = 0;
            ...
            foreach ($merchant as $item){
                ...
                $kinds +=count($item['commodity']);
            }
            return array('list'=>$merchant,'amount'=>$amount,'qty'=>$qty,'weight'=>$weight,'kinds'=>$kinds);
        }
    ...
    {
        $sql = "SELECT goods_id, goods_name, original_price, now_price, goods_image,merchant_id
                FROM {$this->table_name}
    ...
    public function get_bestseller($cate_id = null, $limit = 10)
    {
        ...
        $sql = "SELECT a.goods_id, COUNT(1) AS count, b.goods_name, b.now_price, b.goods_image ,b.merchant_id
