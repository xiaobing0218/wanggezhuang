...
class goods_cate_model extends Model
{
    ...
    {
        if($find_all = $this->find_all(null, 'seq ASC', 'state,cate_id, parent_id, cate_name, seq'))
        {
    ...
    public function goods_cate_bar()
    {
        ...
        if($cates = $this->find_all(array('parent_id' => 0,'state'=>1), 'seq ASC', $field))
        {
            foreach($cates as $k => $v) {
                $cates[$k]['children'] = $this->find_all(array('parent_id' => $v['cate_id'],'state'=>1), 'seq ASC', $field);
                if(!empty($cates[$k]['children'])){
                    foreach($cates[$k]['children'] as $kk => $vv) {
                        $cates[$k]['children'][$kk]['children'] = $this->find_all(array('parent_id' => $vv['cate_id'],'state'=>1), 'seq ASC', $field);
                    }
                }
            }
        }
    ...
    public function set_filters($cate_id, $att = '')
    {
        ...
        //价格筛选
//        $filters['price'] = $this->auto_price_zone($cate_id, 4);
